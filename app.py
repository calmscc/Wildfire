...
# Geolocator setup outside the form for efficiency
...
with st.form(key="fire_form"):
    ...
    # --- Location input for California, auto-geocoded ---
    location_input = cols2[3].text_input("Enter California location (city, landmark, or address)", value="")
...
# Location geocoding and validation only after form submission
LATITUDE = LONGITUDE = None
location_address = None
if submitted:
    if location_input:
        location = geocode(location_input + ", California, USA")  # force California context
        if location and "California" in location.address:
            LATITUDE = location.latitude
            LONGITUDE = location.longitude
            location_address = location.address
            st.write(f"**Detected location:** {location.address}")
            st.map(pd.DataFrame({'lat': [LATITUDE], 'lon': [LONGITUDE]}))
        else:
            st.warning("Location not found or outside California. Please enter a valid California location.")
            # Block further steps unless location is valid
            LATITUDE = LONGITUDE = None
    else:
        st.info("Please enter a California location to determine latitude and longitude.")
        LATITUDE = LONGITUDE = None
